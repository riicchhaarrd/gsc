...
#define buf_push(v, e)                                                                                    \
	...
	{                                                                                                     \
		...
		{                                                                                                 \
			(v) = buf_grow1(v, sizeof(*(v)), !buf_capacity((v)) ? BUF_INIT_CAPACITY : buf_capacity((v))); \
		}                                                                                                 \
		(v)[buf_ptr((v))->size++] = (e);                                                                  \
	} while(0)
...
static void *buf_grow1(void *v, size_t esize, ptrdiff_t n)
{
	...
	if(v)
	{
		...
		if(n > 0 && p->capacity + n > max / esize)
			goto fail; /* overflow */
		p = (struct buf*)realloc(p, sizeof(struct buf) + esize * (p->capacity + n));
		if(!p)
			goto fail;
		p->capacity += n;
	...
	else
	{
		if((size_t)n > max / esize)
			goto fail; /* overflow */
		p = (struct buf*)malloc(sizeof(struct buf) + esize * n);
		if(!p)
			goto fail;
		p->capacity = n;
		p->size = 0;
